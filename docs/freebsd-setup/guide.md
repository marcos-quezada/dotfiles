...
| `sysctl_add_oid: can't re-use a leaf (hw.dri.debug)!` | Cosmetic, confirmed still present in a fresh post-fix `dmesg` capture (2026-09-27) — **not** resolved by the NVIDIA/`tmpfs` fixes as earlier assumed; a single duplicate-leaf warning when both `drmn0` (Intel i915) and the NVIDIA driver each register a DRM device and collide on the same shared `hw.dri.debug` sysctl node. Both devices still attach and work correctly afterward; no functional impact observed |
| Sway startup flashes a red `eglQueryDeviceStringEXT`/`EGL_BAD_PARAMETER` message | Cosmetic, but **caused by today's NVIDIA fix specifically — not a pre-existing, structural condition** (corrected after actually testing, see below). mechanism confirmed via wlroots' own source (`render/egl.c`): during startup it enumerates *every* EGL device and queries each one to find the one matching its already-selected (Intel-only) DRM device; on failure it `continue`s past that device and moves on, exactly as designed, just logged at `ERROR` level. **the causal trigger, confirmed by testing `pre-nvidia-fix`**: in that older boot environment (596.x-branch NVIDIA driver, which never successfully attached to this GPU at all — `"will ignore this GPU"`), no NVIDIA EGL device ever existed for wlroots to query, so the message never appeared there. once `nvidia.ko` was fixed to correctly attach (today's actual goal, for future compute use), NVIDIA started publishing a real EGL device — which wlroots then dutifully enumerates and fails to fully query, every boot. so this message is a direct, expected trade-off of the NVIDIA fix working, not an unrelated wlroots quirk. `WLR_DRM_DEVICES` was tried as a possible fix and reverted — it restricts the DRM *backend*'s device selection, but this query happens in a separate EGL device-matching path unaffected by it. no wlroots env var controls this (checked the full list). not fixable without patching wlroots' log level or hiding the NVIDIA EGL device from the system entirely (higher-risk, not pursued) |
