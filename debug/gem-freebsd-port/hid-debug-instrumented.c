...
static int add_device(const char *path)
{
    ...
    if (g_device_count >= freebsd_hid_max_devices) {
        fprintf(stderr, "[hid-debug] %s: too many devices already\n", path);
        return 0;
    }
    fd = open(path, O_RDONLY | O_NONBLOCK | O_CLOEXEC);
    if (fd < 0) {
        fprintf(stderr, "[hid-debug] %s: open failed: %s\n", path,
                strerror(errno));
        return 0;
    }
    ...
    if (ioctl(fd, EVIOCGBIT(0, sizeof(event_bits)), event_bits) < 0) {
        fprintf(stderr, "[hid-debug] %s: EVIOCGBIT(0) failed: %s\n", path,
                strerror(errno));
        (void)close(fd);
        return 0;
    }
    ...
    fprintf(stderr,
            "[hid-debug] %s: opened ok, has_keyboard=%d has_pointer=%d "
            "EV_KEY=%d EV_REL=%d EV_ABS=%d\n",
            path, device->has_keyboard, device->has_pointer,
            bit_is_set(event_bits, EV_KEY), bit_is_set(event_bits, EV_REL),
            bit_is_set(event_bits, EV_ABS));
    if (!device->has_keyboard && !device->has_pointer) {
        fprintf(stderr,
                "[hid-debug] %s: rejected, neither keyboard nor pointer\n",
                path);
        (void)close(fd);
        return 0;
    }
    ...
    if (device->has_pointer && !device->has_abs_x && !device->has_abs_y &&
        ...
        fprintf(stderr,
                "[hid-debug] %s: rejected, relative pointer superseded by "
                "an absolute one already found\n",
                path);
        (void)close(fd);
        memset(device, 0, sizeof(*device));
        return 0;
    ...
    if (option_enabled("GEM_FREEBSD_GRAB") && ioctl(fd, EVIOCGRAB, 1) < 0) {
        fprintf(stderr, "[hid-debug] %s: EVIOCGRAB failed: %s\n", path,
                strerror(errno));
        (void)close(fd);
        memset(device, 0, sizeof(*device));
        return 0;
    ...
    fprintf(stderr, "[hid-debug] %s: ACCEPTED as device index %zu\n", path,
            g_device_count);
    ++g_device_count;
    return 1;
}
...
static void discover_devices(void)
{
    ...
    if (directory == NULL) {
        fprintf(stderr, "[hid-debug] opendir(/dev/input) failed: %s\n",
                strerror(errno));
        return;
    }
    while ((entry = readdir(directory)) != NULL) {
        ...
        if (snprintf(path, sizeof(path), "/dev/input/%s", entry->d_name) <
            (int)sizeof(path)) {
            fprintf(stderr, "[hid-debug] trying %s\n", path);
            (void)add_device(path);
        }
    }
...
int gem_hid_init(void)
{
    ...
    g_caps_lock = 0;
    g_next_device = 0u;
    fprintf(stderr, "[hid-debug] gem_hid_init done: g_device_count=%zu\n",
            g_device_count);
...
int gem_hid_poll(gem_hid_event_t *event)
{
    ...
    if (first_call) {
        fprintf(stderr, "[hid-debug] gem_hid_poll first call: g_device_count=%zu\n",
                g_device_count);
        first_call = 0;
    }
    ...
    for (checked = 0u; checked < g_device_count; ++checked) {
        ...
        for (;;) {
            ...
            fprintf(stderr,
                    "[hid-debug] translated event type=%d from fd=%d\n",
                    translated.type, device->fd);
