                             ...
                             const struct input_event *input)
{
    ...
    static unsigned long mt_trace_count;

    /*
     * gem-freebsd-touchpad-mtslots task 1: temporary raw-event tracing
     * for every EV_ABS event, to confirm (or rule out) whether pressing
    ...
    if (input->type == EV_ABS && mt_trace_count < 400u) {
        ...
        if (input->code == ABS_MT_SLOT) {
            name = "ABS_MT_SLOT";
        } else if (input->code == ABS_MT_TRACKING_ID) {
            name = "ABS_MT_TRACKING_ID";
        } else if (input->code == ABS_MT_POSITION_X) {
            name = "ABS_MT_POSITION_X";
        } else if (input->code == ABS_MT_POSITION_Y) {
            name = "ABS_MT_POSITION_Y";
        } else if (input->code == ABS_X) {
            name = "ABS_X";
        ...
        ++mt_trace_count;
        fprintf(stderr,
                "[mtslots-debug] EV_ABS code=%u (%s) value=%d\n",
                input->code, name, input->value);
    }
